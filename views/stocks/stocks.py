...
class Stocks(BoxLayout):
    ...
    def add_stocks(self,mv):
        ...
        stock = [mv.ids.productName_input.text.strip(),
    ...
    def delete_from_view(self, ConfirmDialog):
        if self.currentProduct:
            ...
            data.delete_data('stock', where_clause=f"WHERE productName = '{self.currentProduct.productName}'" )
    ...
    def set_update(self, mv):
        ...
        for i, a in enumerate(stock):
            if a == None:
                stock[i] = "None"
        data = Database()
        data.update_data('stock',stock, where_clause=f"WHERE productName = '{self.modifyProduct.productName}'" )
        self.get_stocks()
    @mainthread
